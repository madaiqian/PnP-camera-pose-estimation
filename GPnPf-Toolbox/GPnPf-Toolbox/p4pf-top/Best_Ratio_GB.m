function [f R t] = Best_Ratio_GB(M3D,m2D)
...
%call the best ratio solver (139x153, 14 solutions)
[f zb zc zd] = p4pfr_1_116([glab,glac,glad,glbc,glbd,glcd], m2D(:,1), m2D(:,2), m2D(:,3), m2D(:,4));

%to remove the all zero solutions
index = find(zb.^2+zc.^2+zd.^2>1e-6);
f = f(index);
zb = zb(index);
zc = zc(index);
zd = zd(index);
